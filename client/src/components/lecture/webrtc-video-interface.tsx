...
export default function WebRTCVideoInterface({ lectureId, isTeacher = false }: VideoInterfaceProps) {
  const { user } = useAuth();
  ...
  // WebRTC state
  const [peers, setPeers] = useState<{ [key: number]: BrowserPeer }>({});
  ...
  const addDebugInfo = (message: string) => {
    console.log(`[WebRTCVideo] ${message}`);
  ...
  const startMedia = async () => {
    try {
      ...
      Object.entries(peers).forEach(([peerId, peer]) => {
        // Destroy the old peer and create a new one
        peer.destroy();
        initiatePeerConnection(parseInt(peerId));
      });
    } catch (error) {
  ...
  const initiatePeerConnection = (peerId: number) => {
    ...
    if (!stream) {
      addDebugInfo(`Cannot connect to peer ${peerId}: no local stream`);
      return;
    }
    ...
    try {
      ...
      peer.on('signal', (data: any) => {
        addDebugInfo(`Generated signal for peer ${peerId}`);
  ...
  const processSignal = (data: { peer: number; data: any }) => {
    ...
    if (!stream) {
      addDebugInfo(`Cannot process signal from ${peerId}: no local stream`);
      return;
    }
